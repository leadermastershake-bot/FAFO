...
app.get('/api/database/status', async (req, res) => {
  // A more robust check to see if the database is truly configured
  const dbUrl = process.env.DATABASE_URL;
  if (!dbUrl || dbUrl.includes("placeholder") || dbUrl.includes("mongodb://localhost:27017/metabotprime")) {
    return res.json({ isConfigured: false });
  }
  // Try to connect to the database to be absolutely sure
  try {
